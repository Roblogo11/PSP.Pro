...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    // Listen for auth state changes (handles both PKCE callback and hash fragments)
    ...
    // Also check existing session (handles redirect from /auth/callback)
    const checkSession = async () => {
      const { data: { session } } = await supabase.auth.getSession()
      if (session) {
        setValidToken(true)
      } else {
        // Wait a moment for the auth state listener to fire before showing error
        setTimeout(() => {
          setValidToken(prev => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      {/* Background Glow */}
      <div className="hidden md:block absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[1000px] rounded-full blur-[150px]" style={{ background: 'radial-gradient(circle, rgba(0, 180, 216, 0.15) 0%, transparent 70%)' }} />
