    ...
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 min-h-screen pb-32">
        <div className="max-w-3xl mx-auto px-4 py-12">
          {/* Back */}
          <Link href="/coaches" className="inline-flex items-center gap-2 text-slate-500 dark:text-white/40 hover:text-slate-900 dark:hover:text-white transition-colors text-sm mb-8">
            <ChevronLeft className="w-4 h-4" />
            All Coaches
          </Link>

          {/* Profile Card */}
          <div className="glass-card rounded-2xl border border-cyan-200/40 dark:border-white/10 overflow-hidden mb-6">
            {/* Hero gradient */}
            <div className="h-32 bg-gradient-to-br from-orange/30 via-slate-900 to-cyan/20" />

            <div className="px-6 pb-6">
              {/* Avatar */}
              <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-orange/40 to-cyan/20 flex items-center justify-center -mt-12 mb-4 border-4 border-white dark:border-slate-950 overflow-hidden">
                {coach.avatar_url ? (
                  <Image src={coach.avatar_url} alt={coach.full_name} width={96} height={96} className="w-full h-full object-cover" />
                ) : (
                  <span className="text-4xl font-black text-white">{coach.full_name?.[0] || 'C'}</span>
                )}
              </div>

              <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-1">{coach.full_name}</h1>
              {coach.years_experience && (
                <p className="text-cyan-700 dark:text-white/50 flex items-center gap-1.5 mb-4">
              ...
              <div className="grid grid-cols-2 gap-3 mb-6">
                <div className="bg-cyan-50 dark:bg-white/5 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold text-orange">{stats.totalSessions}+</p>
                  <p className="text-xs text-cyan-700 dark:text-white/50 mt-1">Sessions Coached</p>
                </div>
                <div className="bg-cyan-50 dark:bg-white/5 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold text-cyan">{stats.uniqueAthletes}+</p>
                  <p className="text-xs text-cyan-700 dark:text-white/50 mt-1">Athletes Trained</p>
                </div>
              </div>
              ...
              {coach.specialties && coach.specialties.length > 0 && (
                <div className="mb-6">
                  <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-3">Specialties</h2>
                ...
                <div className="mb-6">
                  ...
                  <ul className="space-y-2">
                    {coach.certifications.map((cert: string) => (
                      <li key={cert} className="flex items-center gap-2 text-slate-700 dark:text-white/70 text-sm">
