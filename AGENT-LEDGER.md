...
### Data loading (the dashboard waterfall)

- **Batch independent queries with `Promise.all`. Never chain them.** Every dashboard page is `'use client'` + `useEffect` → fetch → `setState`, so each sequential `await` is a visible round trip stacked on the last. `useUserStats` alone ran **6 in series**; `/locker` chained 9 across its effects; `/courses` chained 3. All now batched. Only chain when a query genuinely needs a previous result — e.g. `useActiveChild` MUST read `account_type` before fetching children, so it stays serial by design.
- **When adding a query to a hook or page loader, add it to the existing batch** rather than appending another `await`. The comments at each `Promise.all` say so; keep them.
- **Still outstanding (Stage 2, deliberately not done):** the pages render empty then fetch, so users see skeleton-then-content on every navigation. Fixing that means server-rendering first paint on `/locker`, `/booking`, `/progress` — which touches the auth context that impersonation and simulation depend on. Do it as its own change, and test all three modes (normal / impersonating / simulating). Don't mix it with query batching or a regression becomes impossible to attribute.

### Chatbot & tours (they go stale silently)

- **NEVER put a price in the knowledge base. Use a `{{token}}`.** `KNOWLEDGE_BASE` in `src/components/psp-assistant.tsx` holds `{{ELITE_PRICE}}`, `{{ELITE_NAME}}`, `{{BASIC_NAME}}`, `{{DISCOUNT_PCT}}`, `{{INDIVIDUAL_LIST}}`, `{{GROUP_LIST}}`, `{{SERVICE_LIST}}`, `{{DISCOUNT_EXAMPLES}}`. They resolve at answer time from `GET /api/pricing` via `resolvePricingTokens()` (`src/lib/hooks/use-live-pricing.ts`). Change a price in the DB and the chatbot follows automatically. *Why:* it previously hardcoded prices and spent months telling customers membership was **$30/mo** — a price that was never real (not the old $60, not the new $50) — plus six services and three session packages that exist in NO table. Nothing errors when hardcoded copy rots; it just lies about money.
- **On pricing-fetch failure the tokens resolve to "see the Pricing page", never a number.** Keep it that way. A stale figure is worse than no figure.
- **`packages` and `training_packages` are NOT real tables** (both return 400). Never quote session packages. `services` (7 active, $30–$150) and `membership_tiers` are the truth.
- **Dr. Prop's pill must clear the fixed bottom bar on BOTH breakpoints** — `bottom-32` mobile / `sm:bottom-28` desktop. It was `bottom-24 sm:bottom-6` and overlapped the desktop bar by 45px (pill bottom 860 vs bar top 815 at 1440×900). Measure with `getBoundingClientRect()` after changing it; don't eyeball.
