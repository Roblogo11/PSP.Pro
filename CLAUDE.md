...
## 🔁 Self-healing docs (read these FIRST every session)

Three companion docs at the repo root form PSP's self-healing knowledge layer. Read them before doing real work:

- **[AGENT-LEDGER.md](AGENT-LEDGER.md)** — what's TRUE today + the last 5 shipped changes + permanent rules. Updated in the same commit as any shipping change. The "what's the state of things" surface.
- **[COMPONENT-INDEX.md](COMPONENT-INDEX.md)** — auto-generated index of every component / lib / hook / API route. Search HERE before writing something new — chances are good we've already shipped it. Regenerated by the pre-commit git hook (`scripts/git-hooks/pre-commit`).
- **[reference/](reference/)** — the in-repo knowledge library. Bibles for subsystems, recipes for common tasks, lessons for hard-won institutional memory. Closed-loop accumulator: every meaningful session adds new lessons here.

Plus `supabase/migrations/STANDARDS.md` for migration-specific rules.

### First-time clone setup

```bash
npm install
npm run hooks            # symlinks scripts/git-hooks/* into .git/hooks/
npm run hooks:check      # verify hooks installed
npm run index            # regenerate COMPONENT-INDEX.md manually if needed
```

After that, every `git commit` auto-regenerates `COMPONENT-INDEX.md` and stages it. No thought required.
